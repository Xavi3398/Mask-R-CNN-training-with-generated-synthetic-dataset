    ...
    for i in pos_ids:
        ...
        if config.USE_MINI_MASK:
            ...
            placeholder[gt_y1:gt_y2, gt_x1:gt_x2] = \
                np.array(Image.fromarray(class_mask).resize((gt_w, gt_h)))
                #np.round(np.array(Image.fromarray(class_mask.astype(float)).resize((gt_w, gt_h))) / 255.0).astype(bool)
        ...
        mask =np.array(Image.fromarray(m).resize((config.MASK_SHAPE[1], config.MASK_SHAPE[0])))
        #mask =np.array(Image.fromarray(m.astype(float)).resize((config.MASK_SHAPE[1], config.MASK_SHAPE[0]))) / 255.0
        #mask = scipy.misc.imresize(m.astype(float), config.MASK_SHAPE, interp='nearest') / 255.0

        # Resize conversion:
        # IMAGE = m.astype(float)
        # SIZE = (config.MASK_SHAPE[1], config.MASK_SHAPE[0])
        #m = scipy.misc.imresize(IMAGE, SIZE_H_W, interp='bilinear')
        #m = np.array(Image.fromarray(IMAGE).resize(SIZE_W_H))
...
class MaskRCNN():
    ...
    def detect(self, images, verbose=0, min_confidence = None, class_change = None, class_n = None):
        ...
        for i, image in enumerate(images):
            ...
            # Change class_ids to fit dataset
            if class_n != None or class_change != None:

                # change class id values
                for (a, b) in class_change:
                    final_class_ids = np.where(final_class_ids == a, b, final_class_ids)

                # delete non detected classes
                indexes = np.where(final_class_ids > class_n)[0]
                final_rois = np.delete(final_rois, indexes, axis = 0)
                final_class_ids = np.delete(final_class_ids, indexes)
                final_scores = np.delete(final_scores, indexes)
                final_masks = np.delete(final_masks, indexes, axis = 2)
            
            # select only > min_confidence
            if min_confidence != None:
                indexes = np.where(final_scores < min_confidence)[0]
                final_rois = np.delete(final_rois, indexes, axis = 0)
                final_class_ids = np.delete(final_class_ids, indexes)
                final_scores = np.delete(final_scores, indexes)
                final_masks = np.delete(final_masks, indexes, axis = 2)
